...
func (s Streamer) streamVersion1(frame scanner.Frame) {
	...
	packetCount := int(math.Ceil(float64(len(frame.Pixel)) / maxPixelsPerPacket))
	for i := 0; i < packetCount; i++ {
		// get number of pixels in this packet
		pixelCount := len(frame.Pixel) - i*maxPixelsPerPacket
		...
		for c := 0; c < pixelCount; c++ {
			r, g, b, _ := frame.Pixel[i*maxPixelsPerPacket+c].RGBA()
			packet.Write([]byte{
...
func writeHeader(packet *bytes.Buffer, frame scanner.Frame) {
	...
	// total led count
	buffer.WriteByte(0)
	binary.Write(buffer, binary.LittleEndian, uint16(len(frame.Pixel)))
